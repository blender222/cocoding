$(function() {
  var oSVG = Snap('#svgClock')
  // <開始畫圖>
  // 畫白底
  var clockBackground = oSVG.circle(0, 0, 81)
  // 畫數字
  var numberGroup12 = oSVG.g()
  var svgArrayNum = new Array(13)
  for (let i = 1; i <= 12; i++) {
  ...
  // 畫刻度
  var tickGroup48 = oSVG.g()
  var tickGroup12 = oSVG.g()
  var tempTick = null
  for (let i = 1; i <= 60; i++) {
    if (i % 5 == 0) {
      // 畫大刻度
  ...
  // 畫外圓框
  var clockFrame = oSVG.circle(0, 0, 81)
  // 畫指針
  var svgHourPointer = oSVG.line(0, 7, 0, -40)
  var svgMinutePointer = oSVG.line(0, 10, 0, -65)
  var svgSecondPointer = oSVG.line(0, 15, 0, -70)
  var pointerCircle = oSVG.circle(0, 0, 2.2)
  // 定義漸層
  var gradClockFrame = oSVG.gradient('l(0,0,1,1)#f4a460-#daa520')
  // 設置外觀
  svgHourPointer.attr({
    stroke: '#000',
    strokeWidth: 5,
  ...
  drawNowTime()
  // 時鐘動畫定時器
  var drawClockTimer = setInterval(drawNowTime, 1000)
  // 畫現在時間於時鐘
  ...
  // <蕃茄鐘功能>
  var $backgroundRest = $('#backgroundRest')
  var $goTomato = $('#goTomato')
  var $stopTomato = $('#stopTomato')
  var $resetTomato = $('#resetTomato')
  var $bar1 = $('#bar1')
  var $bar2 = $('#bar2')
  var $displayValue1_1 = $bar1.find('.valueBox>.displayValue')
  var $displayValue2_1 = $bar2.find('.valueBox>.displayValue:first-child')
  var $displayValue2_2 = $bar2.find('.valueBox>.displayValue:last-child')
  var $slider1 = $bar1.children('.slider')
  var $slider2 = $bar2.children('.slider')
  var $minutesLeft = $('#countDownText>.minutesLeft')
  var $secondsLeft = $('#countDownText>.secondsLeft')
  // 定義蕃茄物件
  var tomato = {
    // 工作時間與休息時間(分鐘)
    workTime: 25,
  ...
  function goTomato() {
    ...
    // 產生扇形的d字串
    var pathStringWork = makeSector(tomato.workTime, 80, 0, 0)
    var pathStringRest = makeSector(tomato.restTime, 80, 0, 0)
    // 繪出
  ...
  function makeSector(timeLength, r, Cx, Cy) {
    var largeArcFlag = timeLength > 30 ? 1 : 0
    var arcEndX = r * Snap.cos(timeLength * 6)
    var arcEndY = r * Snap.sin(timeLength * 6)
    var pathString = 'M' + Cx + ',' + Cy + 'h' + r + 'A' + r + ',' + r + ',' + 0 + ',' + largeArcFlag + ',' + 1 + ',' + arcEndX + ',' + arcEndY + 'Z'
    return pathString
  }
  // 倒數計時的function
  function countDown() {
    // 計算剩餘秒數
    var remainingSeconds = (tomato.endTime - Date.now()) / 1000
  ...
  function fillBar1(e, $this) {
    // 更新底色
    var percentNum = ($slider1.val() - $slider1.attr('min')) / ($slider1.attr('max') - $slider1.attr('min'))
    $this.find('.fill').css('width', (40 - 3) * percentNum + 'rem')
  }
  // bar2即時填色
  function fillBar2(e, $this) {
    // 更新底色
    var percentNum = ($slider2.val() - $slider2.attr('min')) / ($slider2.attr('max') - $slider2.attr('min'))
    $this.find('.fill').css('width', (40 - 3) * percentNum + 'rem')
  }
  // 即時改變slider上面的顯示值
  ...
  // <雜項功能>
  // 提醒音效(取出的非jQuery物件)
  var soundEffect = $('#soundEffect').get(0)
  // 音效開關監聽
  var $soundSwitch = $('#soundSwitch')
  var $soundSwitchIcon = $('#soundSwitchIcon')
  $soundSwitchIcon.on('click', function() {
    toggleSwitch($soundSwitch, $soundSwitchIcon)
  })
  // 重複蕃茄鐘開關監聽
  var $repeatSwitch = $('#repeatSwitch')
  var $repeatSwitchIcon = $('#repeatSwitchIcon')
